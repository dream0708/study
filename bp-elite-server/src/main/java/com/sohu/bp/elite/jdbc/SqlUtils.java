...
import com.sohu.bp.exceptions.BpException;
import com.sohu.bp.persistence.annotation.Column;
import com.sohu.bp.persistence.annotation.UpdateIgnoreNull;
import com.sohu.bp.elite.util.ClassUtils;
import com.sohu.bp.elite.util.UUIDUtils;
...
public class SqlUtils {
     ...
     * @return bound sql
     */
    public static BoundSql buildInsertSql(Object entity, Criteria criteria, NameHandler nameHandler, String tableName) {
        ...
        if(tableName != null && tableName.length() > 0)
        	...
        	genTableName = nameHandler.getTableName(entityClass, autoFields);
        
        String pkName = nameHandler.getPKName(entityClass);

        StringBuilder sql = new StringBuilder("INSERT INTO ");
        List<Object> params = new ArrayList<Object>();
        sql.append(genTableName);
     ...
     * @return bound sql
     */
    public static BoundSql buildUpdateSql(Object entity, Criteria criteria, NameHandler nameHandler, String tableName) {
        ...
        sql.append("UPDATE ").append(genTableName).append(" SET ");
        ...
        while (iterator.hasNext()) {
            ...
            if (StringUtils.equalsIgnoreCase(primaryName, columnName)) {
                ...
                if (ArrayUtils.isEmpty(values) || StringUtils.isBlank(values[0].toString())) {
                    throw new BpException("主键值不能设为空");
                }
    ...
    protected static Map<String, AutoField> mergeAutoField(Object entity, Criteria criteria,
        ...
        if (criteria != null && !CollectionUtils.isEmpty(criteria.getAutoFields())) {
            ...
            while (iterator.hasNext()) {
                ...
                //如果是update的字段，统一处理字符串的长度
                Object[] values = autoField.getValues();
                if(autoField.getType() == AutoField.FIELD_UPDATE && values != null
                   && values.length > 0){
                	try {
                		Object value = values[0];
                		if(value != null && value instanceof String){
                			String fieldName = autoField.getName();
                			Field field = criteria.getEntityClass().getDeclaredField(fieldName);
                			if(field != null){
                				Column column = field.getAnnotation(Column.class);
                				//通过getMethod获取anotation
                				if(column == null){
                					String methodName = "get"+fieldName.substring(0,1).toUpperCase()+fieldName.substring(1);
                					Method getMethod = criteria.getEntityClass().getDeclaredMethod(methodName, new Class[]{});
                					if(getMethod != null){
                				...
                				if(column != null && column.length() > 0){
                            		...
                            		if(strValue.length() > column.length()){
                            			value = strValue.substring(0, column.length());
        ...
        for (PropertyDescriptor pd : pds) {
        	...
        	Transient annotation = pd.getReadMethod().getAnnotation(Transient.class);
            if(annotation != null)
            	continue;

            String fieldName = pd.getName();

            //null值，忽略 (单独指定的可以指定为null). 对于update语句，需要保留null值.如果设置了UpdateIgnoreNull注解,也忽略
            Object value = getReadMethodValue(pd.getReadMethod(), entity);
            if (value == null) {
            	if(operateType != AutoField.FIELD_UPDATE)
            		continue;
            	else{
            		UpdateIgnoreNull ignoreAnnotation = pd.getReadMethod().getAnnotation(UpdateIgnoreNull.class);
            		if(ignoreAnnotation != null)
            			continue;
            	}
            }
            
            //insert、update语句，统一处理字符串的长度
            if(operateType == AutoField.FIELD_UPDATE && value != null && value instanceof String){
     ...
     * @return
     */
    public static BoundSql buildDeleteSql(Class<?> clazz, Long id, NameHandler nameHandler, String tableName) {
        ...
        if(tableName != null && tableName.length() > 0)
        	...
        	genTableName = nameHandler.getTableName(clazz, fieldMap);
        String sql = "DELETE FROM " + genTableName + " WHERE " + pkName + " = ?";
     ...
     * @return bound sql
     */
    public static BoundSql buildDeleteSql(Object entity, Criteria criteria, NameHandler nameHandler, String tableName) {
        ...
        StringBuilder sql = new StringBuilder("DELETE FROM " + genTableName + " WHERE ");
        BoundSql boundSql = SqlUtils.buildWhereSql(autoFields, nameHandler);
    ...
    public static BoundSql buildByIdSql(Class<?> clazz, Long id, Criteria criteria,
                                        NameHandler nameHandler, String tableName) {
        ...
        if(tableName != null && tableName.length() > 0)
        	...
        	genTableName = nameHandler.getTableName(entityClass, fieldMap);
        ...
        String sql = "SELECT " + columns + " FROM " + genTableName + " WHERE " + pkName + " = ?";
        List<Object> params = new ArrayList<Object>();
        params.add(id);
    ...
    public static BoundSql buildByIdSql(Class<?> clazz, Integer id, Criteria criteria,
            NameHandler nameHandler, String tableName) {
     ...
     * @return bound sql
     */
    public static BoundSql buildQuerySql(Object entity, Criteria criteria, NameHandler nameHandler, String tableName) {
     ...
     * @return bound sql
     */
    public static BoundSql buildListSql(Object entity, Criteria criteria, NameHandler nameHandler, String tableName) {

        BoundSql boundSql = SqlUtils.buildQuerySql(entity, criteria, nameHandler, tableName);
        ...
        if (null != criteria && criteria.isLimit())
        {
        	StringBuilder limitSb = new StringBuilder(" LIMIT ").append(criteria.getStart()).append(",").append(criteria.getCount());
        	boundSql.setSql(boundSql.getSql() + limitSb.toString());
        }
        return boundSql;
    }
     ...
     * @return bound sql
     */
    public static BoundSql buildCountSql(Object entity, Criteria criteria, NameHandler nameHandler, String tableName) {
