...
public class ToolUtil
{
    ...
    public static String getLoginQuestionUrl(String encodedQuestionId, long bpId) {
		String url = ToolUtil.getShortUrl("https://" + OverallDataFilter.askDomain + getQuestionUri(encodedQuestionId));
			try {
			...
			if (null != codeMsgData && codeMsgData.getCode() == ResponseConstants.OK) {
				...
				if (null != dataJSON && dataJSON.containsKey("token")) {
					...
					url = ToolUtil.getShortUrl(new StringBuilder("https://")
							.append(OverallDataFilter.askDomain)
	...
	public static String getLoginAnswerUrl(String encodedAnswerId, long bpId) {
		String url = ToolUtil.getShortUrl("https://" + OverallDataFilter.askDomain + getQuestionUri(encodedAnswerId));
		try {
			CodeMsgData codeMsgData = bpServiceAdapter.getLoginToken(bpId);
			if (null != codeMsgData && codeMsgData.getCode() == ResponseConstants.OK) {
				JSONObject dataJSON = JSONObject.fromObject(codeMsgData.getData());
	...
	public static String getLoginMyFansUrl(long bpId) {
		String url = ToolUtil.getShortUrl("https://" + OverallDataFilter.askDomain + getMyFansUri());
		try {
			CodeMsgData codeMsgData = bpServiceAdapter.getLoginToken(bpId);
			if (null != codeMsgData && codeMsgData.getCode() == ResponseConstants.OK) {
				JSONObject dataJSON = JSONObject.fromObject(codeMsgData.getData());
