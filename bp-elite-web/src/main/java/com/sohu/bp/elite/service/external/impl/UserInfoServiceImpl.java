...
import com.sohu.bp.elite.util.LifeSignatureUtil;
			...
			try
			{
	        	...
	        	for(Expert expert:expertList)
	        	{
	        		if(null != expert && expert.getStatus().equals(ExpertStatus.PASS) && expert.getManageCompanyId() > 0)
	        		{
	        			...
	        			userInfo.setAvatar(ImageUtil.removeImgProtocol(company.getLogo()));
    	...
    	else
    	{
    		...
    		try
    		{
    			...
    			if(null != infoJSON)
    			{
    				...
    				userInfo.setAvatar(ImageUtil.removeImgProtocol(infoJSON.getString("avatar")));
    ...
    public UserInfo getUserInfoByMobile(String mobile) {
        ...
        try{
        	...
        	if (jsonObject.getInt("code") == 0) {
        		...
        		userInfo.setAvatar(ImageUtil.removeImgProtocol(userInfoObject.getString("avatar")));
        		userInfo.setEmail(userInfoObject.getString("email"));
        		userInfo.setMobile(userInfoObject.getString("mobile"));
        		userInfo.setNick(userInfoObject.getString("nick"));
        		return userInfo;
